...
- `hooks/mise_env.lua` — `PLUGIN:MiseEnv(ctx)`. The workhorse. Reads plugin options from `ctx.options` (set by the consumer in `mise.toml`), parses `.envoke-env` (see format below), builds an `envoke <env> --template templates/env.json.j2 --config <cfg> --quiet [--tag …] [--override …]` command, executes it via the injected `cmd` module, parses the resulting JSON, and returns `{ env = [...], cacheable = true, watch_files = [...] }`. Errors use the `[envoke] …` prefix so mise surfaces them with context. Missing `.envoke-env` or `envoke.yaml` is **not** an error: the hook logs a `log.warn`, returns an empty env, and still watches both paths so creating them later invalidates the cache. Malformed content (empty env file, invalid JSON from envoke, missing envoke binary) is still a hard `error(...)`.
